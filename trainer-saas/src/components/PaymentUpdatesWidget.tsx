...
export function PaymentUpdatesWidget() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'cancelled': return <AlertTriangle className="h-4 w-4 text-slate-600" />
      case 'refunded': return <RotateCcw className="h-4 w-4 text-blue-600" />
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'refunded': return 'bg-blue-100 text-blue-700 hover:bg-blue-100'
      ...
      <CardContent>
          ...
          <Card className="mb-4 bg-blue-50 border-blue-200">
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="font-medium text-blue-900">Останнє оновлення</div>
                  <div className="text-sm text-blue-700">Order: {latestUpdate.orderId}</div>
                </div>
              </div>
              <Badge className={getStatusColor(latestUpdate.status)}>{getStatusText(latestUpdate.status)}</Badge>
            </CardContent>
          </Card>
        )}
            ...
            updates.map((update, index) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="font-medium text-slate-900 text-sm">{update.orderId}</div>
                    <div className="text-xs text-slate-500">{new Date(update.timestamp).toLocaleString('uk-UA')}</div>
                  </div>
                </div>
                <Badge className={getStatusColor(update.status)}>{getStatusText(update.status)}</Badge>
              </div>
            ))
          )}
        </div>
        ...
        <div className="flex gap-2 mt-4 pt-4 border-t">
          <Button
            ...
            className="flex-1"
          >
            {isListening ? <><BellOff className="h-4 w-4 mr-2" /> Вимкнути</> : <><Bell className="h-4 w-4 mr-2" /> Увімкнути</>}
          </Button>
          <Button
            variant="outline"
            size="sm"
